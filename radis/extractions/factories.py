...
class FilterFieldFactory(BaseDjangoModelFactory[FilterField]):
    class Meta:
        model = FilterField

    job = factory.SubFactory("radis.extractions.factories.ExtractionJobFactory")
    name = factory.Sequence(lambda n: f"filter_field_{n}")
    description = factory.Faker("sentence", nb_words=10)


class DataFieldFactory(BaseDjangoModelFactory[DataField]):
    class Meta:
        model = DataField

    job = factory.SubFactory("radis.extractions.factories.ExtractionJobFactory")
    name = factory.Sequence(lambda n: f"data_field_{n}")
    description = factory.Faker("sentence", nb_words=10)
    extraction_type = factory.Faker("random_element", elements=[a[0] for a in DataType.choices])
